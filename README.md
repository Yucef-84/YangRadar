...
- 순위 표 열 너비·행 높이 드래그 조절 및 헤더 클릭 정렬
- 수급 수집 부분 저장·재개와 과거 거래일 수집 요청
...
- 설정 조회·저장·인증 테스트 API는 로컬 loopback 요청만 허용하며, API 응답에는 마스킹된 계좌번호만 포함됩니다.
- Base URL은 `https://api.kiwoom.com`(실전) 또는 `https://mockapi.kiwoom.com`(모의)만 사용할 수 있습니다.
...
앱 상단의 `설정` 버튼에서 본인의 키움 REST API 앱키와 시크릿키를 입력합니다. 입력값은 실행 중인 PC의 `.env` 파일에만 저장됩니다. 설정 조회·저장·인증 테스트 API는 실행 PC의 loopback 요청만 허용하며, 저장된 계좌번호는 화면과 API에서 마스킹됩니다. 이미 저장된 계좌번호가 있을 때 계좌번호 입력란을 비워 두면 기존 값이 유지됩니다.
...
종목별 API 요청이 필요해 첫 전체 수집은 오래 걸릴 수 있습니다. 성공한 결과는 50개 단위로 SQLite에 부분 저장하고, 중단 후 다시 갱신하면 이미 저장된 종목은 건너뛰고 실패/미수집 종목만 다시 요청합니다. 화면의 성공·실패 수와 진행률을 보면서 작업할 수 있습니다. 수량 수급 순위 자체에는 금액 수집을 생략해 요청 수를 줄이고, 상세 대시보드에서 금액 수급을 표시합니다. `수집 요청일`을 비워 두면 오늘 날짜를 수집하고, 날짜를 입력하면 해당 거래일을 추가로 백필할 수 있습니다.

순위표의 헤더 오른쪽 경계를 드래그하면 열 너비가 바뀌고, 각 행의 순위 셀 하단 손잡이를 드래그하면 해당 행 높이가 바뀝니다. 조정값은 브라우저 로컬 저장소에 기억됩니다. 화면이 좁아도 가로 스크롤 중 순위와 종목 식별열이 고정되어 현재 행을 확인할 수 있으며, 나머지 열은 가로 스크롤로 접근할 수 있습니다. 헤더를 클릭하면 현재 표시된 순위 100개를 오름차순/내림차순으로 정렬합니다.
...
`전일`이 모두 `신규`이면 직전 저장 기준일이 없거나 해당 종목이 직전 순위에 없다는 뜻입니다. `기준일` 목록은 실제로 SQLite에 저장된 수집일만 보여 주므로 첫 수집 직후에는 날짜가 하나만 보입니다. 다음 거래일 자동 수집 또는 `수집 요청일`을 지정한 백필 후 전일 비교와 날짜 선택지가 늘어납니다.

수급 수집 데이터는 로컬 `data\yangradar.sqlite3`에 저장됩니다. 키움 API가 설정되어 있으면 평일 15:40 KST 이후 당일 수급 데이터의 준비상태를 확인합니다. 표본 데이터가 아직 준비되지 않았으면 약 10분 간격으로 다시 확인하고, 준비되면 당일 전체 수급 수집을 시작합니다. 이미 당일 수집이 완료됐다면 중복 수집하지 않으며, 실패하거나 일부 종목만 저장된 작업은 이후 확인 과정에서 다시 이어서 수집할 수 있습니다. 주말에는 준비상태를 확인하지 않고, 앱이 꺼져 있던 과거 날짜를 자동으로 백필하지 않습니다. 장 마감 시각은 거래소·API 반영 시점에 따라 달라질 수 있으므로, 완료된 결과를 확인한 뒤 사용하세요.
