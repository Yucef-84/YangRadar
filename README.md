...
종목별 API 요청이 필요해 첫 전체 수집은 오래 걸릴 수 있습니다. 성공한 결과는 50개 단위로 SQLite에 부분 저장하고, 중단 후 다시 갱신하면 이미 저장된 종목은 건너뛰고 실패/미수집 종목만 재시도합니다. 화면의 성공·실패 수와 진행률을 보면서 작업할 수 있습니다. 수량 수급 순위 자체에는 금액 수집을 생략해 요청 수를 줄이고, 상세 대시보드에서 금액 수급을 표시합니다. `수집 요청일`을 비워 두면 오늘 날짜를 수집하고, 날짜를 입력하면 해당 거래일을 추가로 백필할 수 있습니다.
