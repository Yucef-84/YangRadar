...
순위표의 헤더 오른쪽 경계를 드래그하면 열 너비가 바뀌고, 각 행의 순위 셀 하단 손잡이를 드래그하면 해당 행 높이가 바뀝니다. 조정값은 브라우저 로컬 저장소에 기억됩니다. 화면이 좁으면 표는 가로 스크롤로 전체 열을 확인할 수 있으며, 헤더를 클릭하면 현재 표시된 순위 100개를 오름차순/내림차순으로 정렬합니다.
