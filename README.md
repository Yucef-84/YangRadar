...
상단의 `수급 순위` 버튼에서 전체 종목의 일별 수급 순위를 확인할 수 있습니다. `전체 수급 갱신` 버튼을 누르면 키움 REST API의 `ka10099`로 KOSPI(`0`), KOSDAQ(`10`), ETF(`8`) 목록을 가져온 뒤 `ka10060` 일별 투자자 데이터를 수집합니다. 종목 수가 많으면 수집에 시간이 걸리며, 화면에서 진행률을 확인할 수 있습니다.
